...
fn bron_kerbosch<'a>(r: &HashSet<&'a str>, p: &HashSet<&'a str>, x: &HashSet<&str>, graph: &HashMap<&'a str, HashSet<&'a str>>) -> Rc<HashSet<&'a str>> {
    if p.is_empty() && x.is_empty() {
        ...
        for &v in p {
            ...
            r_rec.insert(v);
            let p_rec = p_loop.clone().intersection(graph.get(v).unwrap()).copied().collect();
            let x_rec = x_loop.clone().intersection(graph.get(v).unwrap()).copied().collect();
